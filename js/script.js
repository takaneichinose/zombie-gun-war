...
var ONE_SECOND = 1000;
var FPS = 24;
var SCREEN_EDGE = ScrData.Width - Character.Width;
var MOVE_SPEED = 0.8;
var BASIC_UNIT = "vmin";
var TEXT_UNIT = "em";
var FIRE_INTERVAL = 300;
var FIRE_SPEED = 1.2;
var ENDING_ANIMATION = 2000;
var app = new Vue({
    el: "#app",
    data: {
        keys: {
    ...
    methods: {
        ...
        randomize: function (start, end) {
            return Math.round(Math.random() * (end - start) + start);
        },
        // Get the lapsed time
        timeLapse: function (currentTime) {
        ...
        gameLoop: function () {
            ...
            setTimeout(function () {
                if (_this.play === true) {
                    ...
                    _this.moveCharacter(Which[Which.Player]);
                    _this.moveFire(Which[Which.Player]);
                    _this.fireCollision(Which[Which.Player], 0, 0);
                    _this.computerAction();
                    _this.moveCharacter(Which[Which.Computer]);
                    _this.moveFire(Which[Which.Computer]);
                    _this.fireCollision(Which[Which.Computer], 0, 0);
                    if (_this.currentTime - _this.lastRenderTime > ONE_SECOND / FPS) {
                        _this.lastRenderTime = _this.currentTime;
        ...
        initializeCharactersSize: function () {
            var width = Character.Width + BASIC_UNIT;
            var height = Character.Height + BASIC_UNIT;
        ...
        initializeCharactersPosition: function () {
            var end = SCREEN_EDGE * 100;
            var player = this.randomize(0, end) / 100;
            var computer = this.randomize(0, end) / 100;
            this.location.Player = player;
            this.location.Computer = computer;
        },
        // Set the initial fire values
        initializeFire: function () {
            this.fires = {
                Player: [],
                Computer: []
            };
        ...
        initializeFirePosition: function (which) {
            var fireTime = this.currentTime;
            var lastFire = this.lastFireTime[which];
            if (fireTime - lastFire < FIRE_INTERVAL) {
                return;
            }
            if (this.fires[which].length < 2) {
                this.lastFireTime[which] = fireTime;
                var x = this.location[which] + Character.Width / 2 - Fire.Width / 2;
                var y = Character.Height;
        ...
        // Launch the fire
        launchFire: function (which) {
            this.initializeFirePosition(which);
        },
        // Move the fire's location to another point
        moveFire: function (which) {
            var fireSpeed = FIRE_SPEED;
            if (which === Which[Which.Player]) {
                fireSpeed *= -1;
            }
            for (var i in this.fires[which]) {
                this.fires[which][i].y += fireSpeed;
                var fire = this.fires[which][i];
        ...
        renderFires: function () {
            var whichArray = [Which[Which.Player], Which[Which.Computer]];
            for (var i in whichArray) {
                var which = whichArray[i];
                for (var j in this.fires[which]) {
                    var fire = this.fires[which][j];
                    this.fires[which][j].style.top = fire.y + BASIC_UNIT;
                }
            }
        },
        // Fire collision
        fireCollision: function (which, advance, prediction) {
            var otherWhich = Which[Which.Computer];
            var yCharacter = Character.Height;
            var fireHeightHalf = Fire.Height / 2;
            if (which === Which[Which.Computer]) {
                otherWhich = Which[Which.Player];
                yCharacter = ScrData.Height - Character.Height;
            }
            for (var i = 0; i < this.fires[which].length; i++) {
                var fire = this.fires[which][i];
                var x1 = fire.x;
                var x2 = x1 + Fire.Width;
                var y = fire.y;
                var cX1 = this.location[otherWhich];
                var cX2 = cX1 + Character.Width;
        ...
        analyzeFire: function (i, which, advance) {
            var _this = this;
            if (advance === 0) {
                this.fires[which].splice(i, 1);
                this.points[which]++;
            ...
            if (this.points[which] === this.winningScore[this.maxScore]) {
                this.play = false;
                if (which === Which[Which.Computer]) {
                    var aiType = Difficulty[this.ai.difficulty];
        ...
        clearActiveKey: function (key) {
            if (key === this.activeKey.Player) {
                this.activeKey.Player = null;
            }
        },
        // Set the computer's difficulty
        setComputerDifficulty: function () {
            var difficulty = Difficulty[this.ai.difficulty];
        ...
        computerAction: function () {
            ...
            var moveElapsedTime = this.timeLapse(this.ai.moveCurrentTime);
        ...
        computerLaunchFire: function () {
            var fireElapsedTime = this.timeLapse(this.ai.fireCurrentTime);
        ...
        computerDodge: function () {
            var fireIndex = this.fireCollision(Which[Which.Player], this.ai.dodgeThreshold, 0);
            if (fireIndex !== null) {
        ...
        computerAvoidFire: function () {
            var fireIndex = this.fireCollision(Which[Which.Player], this.ai.dodgeThreshold, Fire.Width);
            if (fireIndex !== null) {
                this.activeKey.Computer = null;
                this.computerLaunchFire();
                return true;
        ...
        computerFiringGap: function () {
            var computerLocation = this.location.Computer;
            var playerLocation = this.location.Player;
            var fullWidth = Character.Width;
        ...
        computerDistanceCheck: function (fireIndex) {
            var x = this.location.Computer + Character.Width / 2;
            var f1 = this.fires.Player[fireIndex].x;
            var f2 = f1 + Fire.Width;
            var d1 = Math.abs(x - f1);
            var d2 = Math.abs(x - f2);
            return d1 > d2;
        },
        // Set the game controls for playing status
        setPlayControlsDown: function (key) {
            switch (key) {
                case Keys.Left:
        ...
        setMenuControlsDown: function (key) {
            switch (key) {
                case Keys.Up:
        ...
        // Set the game controls for playing status
        setPlayControlsUp: function (key) {
            switch (key) {
                case Keys.Space:
        ...
        // Set the game controls for menu status
        setMenuControlsUp: function (key) {
            switch (key) {
                case Keys.Space:
                    this.confirmMenu();
                    break;
        ...
        setControls: function () {
            ...
            window.addEventListener("keydown", function (e) {
                if (_this.play === true) {
                    _this.setPlayControlsDown(e.keyCode);
                }
                else {
                    _this.setMenuControlsDown(e.keyCode);
                }
            });
            window.addEventListener("keyup", function (e) {
                if (_this.play === true) {
                    _this.setPlayControlsUp(e.keyCode);
                }
                else {
                    _this.setMenuControlsUp(e.keyCode);
                }
            });
        },
        //------------------------------------------------------------INTERFACE EVENTS
        ...
        selectMenu: function (key) {
            var count = this.stepMenuCount[this.displayStep];
            var option = null;
            if (key === Keys.Up) {
                option = -1;
            }
